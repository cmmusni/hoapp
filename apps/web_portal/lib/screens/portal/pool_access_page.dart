...
class _StaffViewState extends State<_StaffView> {
  ...
  void _openStaffRegistration() {
    showDialog(
      context: context,
      builder: (_) => _StaffRegistrationDialog(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              showDialog(
                context: context,
                builder: (_) => const _MaxPaxSettingsDialog(),
              );
            },
          ),
        ],
...
class _StaffRegistrationDialog extends StatefulWidget {
  final VoidCallback onRegistered;

  const _StaffRegistrationDialog({required this.onRegistered});

  @override
  State<_StaffRegistrationDialog> createState() =>
      _StaffRegistrationDialogState();
}

class _StaffRegistrationDialogState extends State<_StaffRegistrationDialog> {
  final _formKey = GlobalKey<FormState>();
  final _fullNameController = TextEditingController();
  final _phoneController = TextEditingController();
  final _emailController = TextEditingController();
  ...
  List<Unit> _availableUnits = []; // units without existing registrations
  Unit? _selectedUnit;
  OccupantType _occupantType = OccupantType.resident;
                  ...
                  const Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Create a pool registration on behalf of a unit',
                ...
                child: Column(
                  children: [
                    const Icon(Icons.error_outline,
                        size: 48, color: Colors.red),
                    const SizedBox(height: 12),
                    Text('Error loading units: $_loadError'),
                  ],
                ),
              )
            else
...
// ============================================================
// MAX PAX SETTINGS DIALOG
// ============================================================

class _MaxPaxSettingsDialog extends StatefulWidget {
  const _MaxPaxSettingsDialog();

  @override
  State<_MaxPaxSettingsDialog> createState() => _MaxPaxSettingsDialogState();
}

class _MaxPaxSettingsDialogState extends State<_MaxPaxSettingsDialog> {
    ...
    return AlertDialog(
      ...
      title: Row(
        children: [
          ...
          const SizedBox(width: 8),
          const Expanded(
            child: Text('Max Swimmers per Unit Type',
                style: TextStyle(fontSize: 18)),
          ),
          IconButton(
            ...
            : _error != null
                ...
                : _unitTypes == null || _unitTypes!.isEmpty
                    ? const Text(
                        ...
                        'Add unit types in the Households section first.',
                        textAlign: TextAlign.center,
                      )
                    : Column(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          const Text(
                            'Set the maximum number of registered swimmers allowed per unit type.',
